...
mod unit_tests {
    ...
    mod user_input_to_config {
        ...
        fn desktop_folder_is_one_of_the_allowed_desktop_folders() {
            ...
            let config = Config::user_input_to_config(user_input);
            let home_dir_allowed_folder = dirs::home_dir().unwrap().join(".local/share/applications/").to_str().expect("Failed to convert home_dir to string.").to_string();
            let allowed_folders = vec![
                &home_dir_allowed_folder,
